...
- **定義文はこの2つを一字一句このまま使う**（設計 3.1 / 3.2）
  ...
  マップ:
  ```
  壱岐市に関わる計画を全件並べ、階層と計画期間で俯瞰します。構造化データから生成しているので網羅的で常に最新な代わりに、個々の中身には踏み込みません。
  ```
...
```html
<h2>マップ</h2>
<p class="sub">壱岐市に関わる計画を全件並べ、階層と計画期間で俯瞰します。構造化データから生成しているので網羅的で常に最新な代わりに、個々の中身には踏み込みません。</p>
<div class="cards">
...
`plans/koutsuu/`）は手書きのままです。生成物は網羅的で常に最新な代わりに記述が薄くなるので、
