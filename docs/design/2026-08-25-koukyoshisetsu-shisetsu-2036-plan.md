...
```bash
node -e '
const A=292058, P=[22245,19910,18815];
const a36=A*0.8875;
...
// p*11.7 は浮動小数点で 260266.49999999997 になり、四捨五入が1ずれる。
// 十進で正しい 260266.5 を得るため p*117/10 で計算する。
const need = p => Math.round(p*117/10);
for(const p of P) console.log(p, "→ 1人あたり", (Math.round(a36)/p).toFixed(2),
  "／必要面積", need(p), "／削減率", ((A-need(p))/A*100).toFixed(1)+"%");
