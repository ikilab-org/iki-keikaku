 ...
 * 表のセル。
 *
 * data/schema.md の「未調査の表し方」を画面にそのまま出します。
 *   欠落 … まだ調べていない → 「未確認」
...
${g.plans.map(row).join('\n')}
  ...
  空欄と区別できるように書き分けています。計画名にリンクがあるものは、その計画を掲載しているページ（策定予定のものは所管機関のページ）に繋がります。</p></div>
${groups}
</section>`
}
...
function headerBlock(m) {
  ...
  <div>
    ...
    <strong>分野ごとの掘り下げはノートの役割です。</strong>ここには構造だけを置いています。</p>
  </div>
